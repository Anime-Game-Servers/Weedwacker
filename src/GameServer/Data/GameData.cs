...
namespace Weedwacker.GameServer.Data
{
	internal static class GameData
	{
		...
		public static ConfigAbilityPath AbilityPathMap { get; private set; }
		...
		public readonly static Dictionary<uint, ItemConfig> ItemDataMap = new(); // id ItemData is subclassed, and loaded as MaterialData, ReliquaryData, and WeaponData
		...
		public readonly static Dictionary<uint, MonsterExcelConfig> MonsterDataMap = new(); // id
		...
		public readonly static Dictionary<string, ConfigTalentMixin[]> RelicAffixConfigDataMap = new(); // openConfig
		...
		static async Task<T?> LoadObjects<T>(FileInfo fi)
		{
			...
			//try
			//{
			//	var ret = JsonSerializer.Deserialize<T>(str, options);
			//	return ret;
			//}
			//catch (Exception)
			//{
		...
		{
			string data = "Weedwacker.GameServer.Data";
			IEnumerable<Type> q = Assembly.GetExecutingAssembly().GetTypes().Where(t => t.Namespace != null && t.Namespace.StartsWith(data));
			...
			await Task.WhenAll(new Task[]
			{
				...
				LoadExcel(excelPath, o => o.fetter_id, FetterInfoDataMap),
				LoadExcel(excelPath, o => o.fetter_id, FettersDataMap),
				LoadExcel(excelPath, o => o.fetter_id, FetterStoryDataMap),
				...
				LoadExcel<ItemConfig, uint, MaterialData>(excelPath, o => o.id, ItemDataMap),
				...
				LoadExcel(excelPath, o => o.fetter_id, PhotographExpressionDataMap),
				LoadExcel(excelPath, o => o.fetter_id, PhotographPosenameDataMap),
				LoadExcel(excelPath, o => o.proudSkillId, ProudSkillDataMap),
				LoadExcel<ItemConfig, uint, ReliquaryData>(excelPath, o => o.id, ItemDataMap),
				...
				LoadExcel<ItemConfig, uint, WeaponData>(excelPath, o => o.id, ItemDataMap),
			...
			foreach (var container in ConfigAbilityAvatarMap.Values)
			{
				...
				{
					tasks.Add(config.Default.Initialize());
				}
			}
			await Task.WhenAll(tasks);
